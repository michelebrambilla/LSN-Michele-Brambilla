...
void System :: move(int i){ // Propose a MC move for particle i
  if(_sim_type == 3){ //Gibbs sampler for Ising
    ...
    double sum_spin = (_particle(this->pbc(i-1)).getspin() + _particle(this->pbc(i+1)).getspin());
    double prob = 1./(1. + exp(2.*_beta*(_J*sum_spin + _H)));
    double new_spin = (_rnd.Rannyu() < prob ? -1 : +1);
    _particle(i).setspin(new_spin);
    _naccepted++;
...
//      vy(i)=0.0;
//      vz(i)=0.0;
//      double v_value = (_rnd.Rannyu() > 0.5 ? sqrt(3*_temp) : -sqrt(3*_temp));
//      int axis = int(_rnd.Rannyu(0.0, 3.0));
//        if(axis == 0) vx(i) = v_value;
//        else if(axis == 1) vy(i) = v_value;
//        else if(axis == 2)vz(i) = v_value;
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        input>>_n_bins_v;
        _nprop += _n_bins_v;
        _bin_size_v = 4.0*sqrt(_temp)/(double)_n_bins_v; // TO BE FIXED IN EXERCISE 4   // DONE
        _measure_pofv = true;
...
void System :: measure(){ // Measure properties
  ...
  double virial=0.0;
  double chi=0.0; // creata per simmetria col resto
  ...
  if (_measure_magnet){
    ...
    for (int i=0; i<_npart; i++){
    magnetization += double(_particle(i).getspin());  //per U ha diviso per _npart, forse da fare anche qua
    }
  ...
  if (_measure_cv){ 
    ...
    u_temp = tenergy_temp;
      ...
      for (int i=0; i<_npart; i++){
        s_i = double(_particle(i).getspin());
        s_j = double(_particle(this->pbc(i+1)).getspin());
        u_temp += (- _J * s_i * s_j - 0.5 * _H * (s_i + s_j)) / _npart;
      }
    }

      _measurement(_index_cv) = u_temp*u_temp;
...
// TO BE FIXED IN EXERCISE 6
  ...
  if (_measure_chi){
    ...
    if(_measure_magnet)
    chi = _beta*pow(magnetization*_npart, 2)/_npart;
    ...
    chi = _beta*pow(spin_sum, 2)/_npart;
    }  
...
void System :: averages(int blk){
  ...
  if(_measure_gofr){
    for(int i = 0; i < _n_bins; i++){
      ...
      double delta_v = 4./3.*M_PI*(pow((r + _bin_size),3) - pow(r, 3));
      double norm = 1./(_rho*_npart*delta_v);
  ...
  if (_measure_gofr){
    ...
    coutf.close();
    if (blk == _nblocks){
      coutf.open("../OUTPUT/final_gofr.dat",ios::app);
      for(int i = 0; i< _n_bins; i++){
        sum_average = _global_av(_index_gofr + i);
        sum_ave2 = _global_av2(_index_gofr + i);
         double g = (i + 0.5)*_bin_size;
  ...
  if (_measure_cv){
    ...
    double u2  = _average(_index_cv);
    double u = _average(_index_tenergy);

    average = _beta*_beta*(u2 - u*u)*_npart;

    _global_av(_index_cv) += average - u2;
    _global_av2(_index_cv) += average*average - u2*u2;
