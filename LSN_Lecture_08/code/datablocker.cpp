...
double DataBlocker::FinalResults(double& err) const {
    ...
    double progressive_average = cumulative_sum / N;
    double square_progressive_average = square_cumulative_sum / N;
    err = error(square_progressive_average, progressive_average, N);

    return progressive_average;


}

void DataBlocker::Reset(){
    block_average.clear(); 
    current_sum = 0.0;    
    current_steps = 0;
